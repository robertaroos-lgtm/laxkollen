(function(){
  ...
  let store = loadStore(); migrateStore();
  ...
  const isExamInput = $("is-exam");
  const addBtn = $("add");
  const addQuickBtn = $("add-quick");
  const inputRow = $("input-row");
  const chips = Array.from(document.querySelectorAll(".chip"));
  const subjectSummary = $("subject-summary");
  const listEl = $("list");
  const historyNotice = $("history-notice");

  const modalBackdrop = $("modal-backdrop");
  const manageBtn = $("manage-children");
  const childrenList = $("children-list");
  const newChildName = $("new-child-name");
  const newChildSubjects = $("new-child-subjects");
  const saveNewChildBtn = $("save-new-child");
  ...
  const editIsExam = $("edit-is-exam");
  const editDue = $("edit-due");
  const editTimes = $("edit-times");
  const editCancel = $("edit-cancel");
  const editSave = $("edit-save");
  let editingId = null; // single declaration

  function safe(fn){ try{ fn(); } catch(err){ console.error(err); showToast("Fel i skript: "+(err?.message||err), 2600); } }
  ...
  function init(){
    ...
    cancelModalBtn.onclick = ()=>{
      if(modalSnapshot){ store = modalSnapshot; modalSnapshot = null; saveStore(); renderAll(); showToast("Ändringar ångrade"); }
  ...
  function state(){
    ...
    const s = store.children[name]; if(!("filter" in s)) s.filter="active"; if(!("focusedSubject" in s)) s.focusedSubject=""; return s;
  }
  ...
  function addHomework(){
    ...
    renderAll(); showToast(isExam ? "Prov tillagt" : "Läxa tillagd");
  }

  function tick(id){
    const s = state();
    const t = s.todos.find(x=>x.id===id); if(!t) return;
    if(t.isExam){ showToast("Prov försvinner automatiskt efter provdatum"); return; }
    ...
    if(t.timesLeft > 1){
      ...
      t.timesLeft = 0; t.done = true; t.completedOn = todayLocalISO();
      saveStore(); renderAll(); showToast("Läxa klar!");
    }
  }
  ...
  function openEditModal(id){
    ...
    s.subjects.forEach(sub=>{ const opt=document.createElement("option"); opt.value=sub; opt.textContent=sub; editSubject.appendChild(opt); });
  ...
  function saveEditChanges(){
    ...
    const s = state();
    const t = s.todos.find(x=>x.id===editingId); if(!t) return;
    ...
    if(newIsExam){ t.timesTotal = 1; t.timesLeft  = t.done ? 0 : 1; }
    else {
    ...
    saveStore(); closeEditModal(); renderAll(); showToast("Uppgift uppdaterad");
  }
  ...
  function renderChildSelect(){
    ...
    Object.keys(store.children).sort().forEach(name=>{ const opt=document.createElement("option"); opt.value = name; opt.textContent = name; childSelect.appendChild(opt); });
  ...
  function renderSubjectOptions(){
    ...
    if(s.subjects.length===0){ const opt=document.createElement("option"); opt.value=""; opt.textContent="(Lägg till barn & ämnen via 'Hantera barn')"; subjectSelect.appendChild(opt); }
    else { ALL_SUBJECTS.filter(sub=>s.subjects.includes(sub)).forEach(sub=>{ const opt=document.createElement("option"); opt.value=sub; opt.textContent=sub; subjectSelect.appendChild(opt); }); }
  }
  ...
  function renderList(){
    ...
    items.forEach(t=>{
      ...
      li.appendChild(icCheck); li.appendChild(icTrash); li.appendChild(card);
  ...
  function openModal(isOnboarding=false){
    ...
    newChildName.value=""; newChildSubjects.innerHTML=""; buildSubjectsRows(newChildSubjects);
    const lock = !hasAnyChild(); closeModalBtn.disabled = lock; cancelModalBtn.disabled = lock;
    modalBackdrop.removeAttribute("hidden");
  }
  function closeModal(){
    if(!hasAnyChild()){ return; }
    const selections = {}; Object.keys(store.children).forEach(n=> selections[n]=[]);
    const activeChips = Array.from(document.querySelectorAll('#children-list .sub-chip.active[data-child]'));
    activeChips.forEach(btn=>{ const nm = btn.dataset.child; (selections[nm] ||= []).push(btn.dataset.sub); });
    Object.entries(selections).forEach(([childName, subjects])=>{ if (store.children[childName]) store.children[childName].subjects = subjects; });
    saveStore(); renderAll(); showToast("Ämnen sparade");
    modalSnapshot = null; modalBackdrop.setAttribute("hidden","");
  }
  function renderChildrenEditor(){
    childrenList.innerHTML=""; if(!hasAnyChild()) return;
    Object.entries(store.children).forEach(([name, obj])=>{
      const wrap=document.createElement("div"); wrap.className="child-row";
      const nameEl=document.createElement("div"); nameEl.style.minWidth="140px"; nameEl.innerHTML=`<strong>${name}</strong>`;
      const coll=document.createElement("div"); coll.className="subjects-collection"; buildSubjectsRows(coll, obj.subjects, name);
      ...
      const delBtn=document.createElement("button"); delBtn.className="ghost danger-text"; delBtn.textContent="Ta bort barn"; delBtn.title="Tar bort barnet och alla dess läxor"; delBtn.onclick=()=>deleteChild(name);
      actions.appendChild(delBtn);
      wrap.appendChild(nameEl); wrap.appendChild(coll); wrap.appendChild(actions); childrenList.appendChild(wrap);
    });
  }
  function buildSubjectsRows(container, preselected=[], childName=null){
    SUBJECT_GROUPS.forEach(group=>{
      ...
      group.items.forEach(sub=>{
        const btn=document.createElement("button"); btn.type="button"; btn.className="sub-chip"; btn.dataset.sub = sub; if(childName) btn.dataset.child = childName;
        const selected = !!(preselected && preselected.includes(sub)); if(selected) btn.classList.add("active");
        btn.setAttribute("aria-pressed", selected ? "true" : "false");
        btn.textContent = `${subjectIcons[sub]||"📘"} ${sub}`;
        btn.onclick = ()=>{ const ns = !btn.classList.contains("active"); btn.classList.toggle("active", ns); btn.setAttribute("aria-pressed", ns ? "true" : "false"); };
        row.appendChild(btn);
      });
      gwrap.appendChild(header); gwrap.appendChild(row); container.appendChild(gwrap);
    });
  }
  function addNewChild(){
    const name = newChildName.value.trim(); if(!name){ alert("Ange ett namn."); return; }
    ...
    store.currentChild = name; saveStore(); renderAll(); renderChildrenEditor();
    closeModalBtn.disabled = false; cancelModalBtn.disabled = false; showToast(`Lade till ${name}`);
    newChildName.value = ""; newChildSubjects.innerHTML = ""; buildSubjectsRows(newChildSubjects);
  }
  function deleteChild(name){
    ...
    store.currentChild = names.length? names[0] : "";
    if(names.length===0){ closeModalBtn.disabled = true; cancelModalBtn.disabled = true; }
    saveStore(); renderAll(); renderChildrenEditor();
  }
  ...
  function showToast(msg, ms=1200){ const el=document.getElementById('toast'); if(!el) return; el.textContent=msg; el.classList.add('show'); clearTimeout(window._toastTimer); window._toastTimer=setTimeout(()=> el.classList.remove('show'), ms); }
  ...
  function attachSwipe(li, onRight, onLeft){
    ...
    const start = (x, evt)=>{ 
      ...
      dragging=true; startX=x; card.style.transition='none'; li.classList.remove('swipe-left','swipe-right');
    };
    const move  = (x)=>{ if(!dragging) return; currentX = x - startX; card.style.transform = `translateX(${currentX}px)`; if(currentX>0){ li.classList.add('swipe-right'); li.classList.remove('swipe-left'); } else if(currentX<0){ li.classList.add('swipe-left'); li.classList.remove('swipe-right'); } };
    const end   = ()=>{ if(startedOnInteractive){ startedOnInteractive=false; return; } if(!dragging) return; dragging=false; card.style.transition=''; if(currentX > THRESHOLD){ haptic('success'); onRight?.(); reset(); } else if(currentX < -THRESHOLD){ haptic('heavy'); onLeft?.(); } else { reset(); } currentX=0; };
    const reset = ()=>{ card.style.transform='translateX(0)'; li.classList.remove('swipe-left','swipe-right'); };
