...
function getSubjectIcon(sub){
  ...
  return store.customSubjects[sub] || subjectIcons[sub] || '📘';
}
...
const cancelNewChildBtn = document.getElementById('cancel-new-child'); // Spara i editorläge
...
function wireEvents() {
  ...
  // Egna ämnen: lägg till / spara / avbryt
...
function onCancelChildEditor() {
  // Avbryt lägg till/redigera barn och gå tillbaka till listläget
  editingChildSubjectsName = null;
  ...
  // Avbryt ev. pågående redigering av eget ämne
...
function addNewChild() {
  ...
  // Första barnet: välj barnet. Vid 2+ barn: välj ALLA.
...
function addHomework() {
  ...
  // Efter tillägg: återgå automatiskt till ALLA (endast om det finns 2 eller fler barn)
...
function renderChildSelect() {
  ...
  // 1 barn: ingen "ALLA" och barnet är alltid förvalt
  if (names.length === 1) {
    const only = names[0];
  ...
  // 2+ barn: "ALLA" finns och kan vara förvalt
...
// ISO-vecka (mån–sön) + svenska veckodagar
...
function isWeekAfterNextFromISO(targetISO, nowISO) {
  const now = isoToDate(nowISO);
  const target = isoToDate(targetISO);
  // Kontrollera om target ligger i veckan efter nästa (week+2) även vid årsskifte
  ...
  // week+2 definieras som veckan som börjar ~14 dagar fram, men robust via isoWeek jämförelse med +14
...
function computeDueClass(t,todayISO){
  ...
  if(!isWeekend){
    if(isSameISOWeekFromISO(t.due, todayISO)) return 'week-current'; // grön
    if(isNextISOWeekFromISO(t.due, todayISO)) return 'week-next'; // blå
  } else {
    // Helg: nästkommande vecka grön, veckan därefter blå
...
function renderCustomSubjectsUI(){
  ...
  if(!custom.length){
    ...
    p.textContent='Inga egna ämnen än.';
...
function cancelCustomSubjectEdit(){
  ...
  if (customSubAddBtn) customSubAddBtn.textContent = 'Lägg till';
...
function onAddOrSaveCustomSubject(){
  ...
  if(!name){ alert('Skriv ett ämnesnamn.'); return; }
  if(name.length > 24){ alert('Ämnesnamnet får vara max 24 tecken.'); return; }

  // Dubbletter mot inbyggda + befintliga
  if (ALL_SUBJECTS.includes(name) && editingCustomSubjectOldName !== name){
    alert('Det finns redan ett ämne med det namnet.');
    return;
  }
  if (store.customSubjects[name] && editingCustomSubjectOldName !== name){
    alert('Det finns redan ett eget ämne med det namnet.');
    return;
  }
  ...
  if(editingCustomSubjectOldName){
    ...
    if(old !== name){
      ...
      store.customSubjects[name] = icon || oldIcon || '📘';

      // uppdatera barns ämnen + todos
      ...
      store.customSubjects[name] = icon || store.customSubjects[name] || '📘';
    }
  } else {
    store.customSubjects[name] = icon || '📘';
    // markera som valt i editorn (lägga till-läge)
    const chip = newChildSubjects?.querySelector(`.sub-chip[data-sub="${CSS.escape(name)}"]`);
    if(chip){
      chip.classList.add('active');
      chip.setAttribute('aria-pressed','true');
    } else {
      // kommer med i nästa render
    }
  }
...
function deleteCustomSubject(name){
  ...
  // räkna hur många läxor/prov som använder ämnet
  let usedCount = 0;
  Object.values(store.children || {}).forEach(c=>{
    (c.todos||[]).forEach(t=>{
  ...
  if(usedCount > 0){
    const ok = confirm(
      `Ämnet "${name}" används i ${usedCount} läxa/prov.

` +
      `Vill du flytta dessa till "Övrigt" och ta bort ämnet?

` +
      `Tryck OK för att flytta till Övrigt, eller Avbryt för att avbryta.`
    );
    if(!ok) return;
  } else {
    if(!confirm(`Ta bort ämnet "${name}"?`)) return;
  }

  delete store.customSubjects[name];

  // ta bort från barns ämnen och (om det används) flytta todos
  Object.values(store.children || {}).forEach(c=>{
    if(Array.isArray(c.subjects)){
      c.subjects = c.subjects.filter(s=> s!==name);
      // Om barnet nu saknar ämnen: säkra upp med Övrigt
      if(c.subjects.length === 0) c.subjects.push('Övrigt');
    }
    if(usedCount > 0){
      (c.todos||[]).forEach(t=>{
        if(t.subj===name) t.subj='Övrigt';
      });
    }
  });
  ...
  // Bygg om ämneschips i editorn (behåll valda ämnen där det går)
  const pre = getSelectedSubjectsFromEditor().filter(s=>s!==name);
  if(pre.length === 0) pre.push('Övrigt');
