...
function onCheckboxChange(){} /* checkbox borttagen */
...
function closeEditModal(){ editingId = null; editBackdrop.style.display="none"; }
...
function renderSummary(){
  ...
  totalCard.title = "Visa aktiva läxor (alla ämnen)";
  ...
  ALL_SUBJECTS.filter(sub=>s.subjects.includes(sub)).forEach(sub=>{
    ...
    card.className="card"; card.title=`Visa aktiva i ${sub}`; card.setAttribute("role","button"); card.setAttribute("tabindex","0");
    const isSelected = (focused===sub);
    card.innerHTML = `
      <div class="title">${subjectIcons[sub]||"📘"} ${sub}</div>
...
function closeModal(){
  ...
  modalSnapshot = null; modalBackdrop.style.display="none";
}
...
function deleteChild(name){
  ...
  if(names.length===0 && modalBackdrop.style.display==="flex"){ closeModalBtn.disabled = true; cancelModalBtn.disabled = true; }
...
function attachSwipe(li, onRight, onLeft){
  ...
  const THRESHOLD = 72; // px för att trigga
  ...
  const start = (x, evt)=>{ 
    ...
    if(startedOnInteractive) return; // låt knappar fungera
  ...
  const end   = (evt)=>{ if(startedOnInteractive){ startedOnInteractive=false; return; } if(!dragging) return; dragging=false; card.style.transition=''; if(currentX > THRESHOLD){ haptic('success'); onRight?.(); reset(); } else if(currentX < -THRESHOLD){ haptic('heavy'); onLeft?.(); } else { reset(); } currentX=0; };
