...
function computeDueLabel(t,todayISO,dateNice){ if(!t.due) return 'Ingen deadline'; if(t.isExam && t.due<todayISO) return `Provdatum: ${dateNice}`; if(!t.done && t.due===todayISO) return 'Idag'; if(!t.done && t.due<todayISO) return 'Försenad'; return `Till: ${dateNice}`;}
